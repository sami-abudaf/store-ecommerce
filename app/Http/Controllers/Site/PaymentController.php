...
//
        ...
        return view('front.cart.payments',$data , compact('amount'));
    }
    ...
    public function processPayment(Request $request)
    {
        ...
        $ccMon = $ccExp[0];
        $ccYear = $ccExp[1];
        $customerMobile = strlen($phone) <= 11 ? $phone : '12345678';
        $data['Language'] = 'en';
        ...
        $payment_url =   json_decode((string)$response, true);
        ...
        //$json = json_decode((string)$response, true);
        $PaymentId = json_decode((string)$response, true);
        try {
            DB::beginTransaction();
